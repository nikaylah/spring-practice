...
public class UserController {
	...
	public ResponseEntity<?> registerUser(@RequestBody User user){
		...
		try {
			...
			response = new ResponseEntity<String>(HttpStatus.OK); //succssfully added to database with ok
			
		}catch(UserExistsException u) {
			response = new ResponseEntity<String> (HttpStatus.CONFLICT);//there is a conflict
		
			
		}
		catch(Exception e) {
			response = new ResponseEntity<String> (HttpStatus.INTERNAL_SERVER_ERROR);//server error
	...
	public ResponseEntity<?> login(@RequestBody User user) throws UserNotFoundException {
		...
		String token = Jwts.builder().setId(validUser.getUsername())
				...
				.signWith(SignatureAlgorithm.HS256, "userkey").compact(); //(tokentype,tokenname)
